...
@router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    summary="Ingest multiple log enteries",
)
async def create_log_bulk(
    payload: LogBulkCreate, session: AsyncSession = Depends(get_db_session)
) -> list[LogRead]:
    service = IngestionService(session)
...
async def get_service_count(session: AsyncSession = Depends(get_db_session)):
    result = await session.execute(select(func.count(func.distinct(Log.service))))

    return {"service_count": result.scalar_one()}


@router.get("/{log_id}", response_model=LogRead, summary="Get a single log by ID")
async def get_log_by_id(
    log_id: int, session: AsyncSession = Depends(get_db_session)
) -> LogRead:
...
@router.get(
    "/{log_id}/similar",
    response_model=SimilarLogsResponses,
    summary="find logs similar to existing log",
)
async def get_similar_log(
    ...
    return SimilarLogsResponses(
        results=[to_similar_log_result(match) for match in matches]
    )
...
async def upload_logs(
    ...
    for item in raw_logs:
        try:
            valid_logs.append(LogCreate.model_validate(item))
        except Exception:
            failed_count += 1
    ...
    created_logs = (
        await service.create_logs_from_items(valid_logs) if valid_logs else []
    )
