...
class SemanticLogSearchService:
    ...
    def _rank_candidate_logs(
        self, query_text: str, candidate_logs: list[Log], top_k: int
    ):
        ...
        for rank in ranked_results:
            matches.append(
                SemanticLogMatch(log=candidate_logs[rank.index], score=rank.score)
            )
    ...
    async def search(
        ...
        return self._rank_candidate_logs(
            query_text=query, candidate_logs=candidate_logs, top_k=top_k
        )
